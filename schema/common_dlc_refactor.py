...
class Camera(dj.Lookup):
    definition = """ # Lookup table for different cameras / views
    camera_id : int  # attribute for distinguishing between separate cameras/camera angles
    ---
    camera_name  : varchar(128)         # short camera identifier
    position_details : varchar(1048)    # longer description of camera position and properties
    """
    contents = [
        [0, 'Jithin_Forelimb', 'Camera recording the mouse forelimb from the side, while running on the wheel'],
        [1, 'Jithin_Hindlimb', 'Camera recording the mouse hindlimb from the side, while running on the wheel']
    ]
...
class Video(dj.Manual):
    ...
    -> common_exp.Session
    -> Camera
    video_id    : int     # id for keeping track of multiple videos per session
    ---
    filename_video        # path to the video file, relative to session folder
    """

    def get_paths(self) -> List[Path]:
        return utils.get_paths(self, "filename_video")
    def get_path(self, check_existence: bool) -> Path:
...
class VideoInfo(dj.Computed):
    ...
    first_frame    : blob@datastore   # first frame of the video
    """

    def make(self, key: dict) -> None:
        # open file
        video_path = (Video & key).get_path(check_existence=True)
...
class FFMPEGParameter(dj.Lookup):
    ...
    FFMPEG_id           : int           # id of this parameter set
    ---
...
class DLCModel(dj.Lookup):
    ...
    """
    contents = [{"model_name": "DigitsLateralView-mpanze-2021-10-27",
...
class DeepLabCut(dj.Computed):
    definition = """ # deeplabcut processed videos
    -> VideoInfo
    -> DLCModel
    -> FFMPEGParameter
    ---
    iteration          : tinyint      # model iteration which was used to generate predicitons
    """
    class Joint(dj.Part):
        definition = """ # joint predictions from processed videos
        -> Deeplabcut
        ---
        x          : blob@datastore
        y          : blob@datastore
...
class ProcessingParameters(dj.Lookup):
    definition = """ # Parameters for further processing of traces
    """
    
@schema
class ProcessedTraces(dj.Computed):
    definition = """ # Applies interpolation, smoothing and resampling to traces
    -> Deeplabcut
    -> ProcessingParamaters
    """
    class Joint(dj.Part):
        definition = """ # processed joint traces
        -> ProcessedTraces
        ---
        x           : blob@datastore
        y           : blob@datastore
